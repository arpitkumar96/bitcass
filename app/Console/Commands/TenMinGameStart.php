...
class TenMinGameStart extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:ten-min-game-start';
    ...
    public function handle()
    {
        ...
        $ten_min_games = Game::where('is_active','1')->where('duration',10)->get();
        foreach($ten_min_games as $ten_min_game) {
            $total_this_game = StartGame::where('game_id',$ten_min_game->id)->whereDate('created_at', Carbon::today())->count();

            $start_game = new StartGame;
            $start_game->start_game_id = date('Ymd').'0'.($ten_min_game->id.'0000')+$total_this_game+1;
            $start_game->game_id = $ten_min_game->id;
            $start_game->duration = $ten_min_game->duration;
            $start_game->save();
        }
    }
}
