...
class ThreeMinGameStart extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:three-min-game-start';
    ...
    {
        $old_start_games = StartGame::where('duration',3)->where('is_running','1')->get();
        ...
        foreach ($old_start_games as $key => $old_start_game) {
            ...
            foreach ($game_participants as $game_participant) {
                ...
                if($is_win == '1'){
                    ...
                    $user_wallet->type_id = $old_running_game->id;
        ...
        $three_min_games = Game::where('is_active','1')->where('duration',3)->get();
        foreach($three_min_games as $three_min_game) {
            $total_this_game = StartGame::where('game_id',$three_min_game->id)->whereDate('created_at', Carbon::today())->count();

            $start_game = new StartGame;
            $start_game->start_game_id = date('Ymd').'0'.($three_min_game->id.'0000')+$total_this_game+1;
            $start_game->game_id = $three_min_game->id;
            $start_game->duration = $three_min_game->duration;
            $start_game->save();
        }
    }
}
