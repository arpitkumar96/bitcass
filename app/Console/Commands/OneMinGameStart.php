...
class OneMinGameStart extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:one-min-game-start';
    ...
    {
        $old_start_games = StartGame::where('duration',1)->where('is_running','1')->get();
        ...
        $one_min_games = Game::where('is_active','1')->where('duration',1)->get();
        foreach($one_min_games as $one_min_game) {
            $total_this_game = StartGame::where('game_id',$one_min_game->id)->whereDate('created_at', Carbon::today())->count();

            $start_game = new StartGame;
            $start_game->start_game_id = date('Ymd').'0'.($one_min_game->id.'0000')+$total_this_game+1;
            $start_game->game_id = $one_min_game->id;
            $start_game->duration = $one_min_game->duration;
            $start_game->save();
        }
    }
}
