...
class FiveMinGameStart extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:five-min-game-start';
    ...
    {
        $old_start_games = StartGame::where('duration',5)->where('is_running','1')->get();
        ...
        $five_min_games = Game::where('is_active','1')->where('duration',5)->get();
        foreach($five_min_games as $five_min_game) {
            $total_this_game = StartGame::where('game_id',$five_min_game->id)->whereDate('created_at', Carbon::today())->count();

            $start_game = new StartGame;
            $start_game->start_game_id = date('Ymd').'0'.($five_min_game->id.'0000')+$total_this_game+1;
            $start_game->game_id = $five_min_game->id;
            $start_game->duration = $five_min_game->duration;
            $start_game->save();
        }
    }
}
