...
@section('content')
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="col-6 text-right">
            <ol class="mt-4">
                <input type="checkbox" name="subordinate_joinning_status" id="subordinate_joinning_status" data-toggle="toggle" onchange="getJoinningStatus()" @if(optional($subordinate_joinning)->status == '1') checked @endif> <br>
                @error('status')
            ...
            <div class="col-lg-12">
                <div class="ibox ">
                    <div class="ibox-content">
                        <form action="{{route('admin.subordinate.joinning.setting.store')}}" method="POST" class="form-example" id="add_form">
                            @csrf
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="level" id="level" class="form-control select" onchange="getCommissionTable()" required>
                                            @foreach ($levels as $level)
                                                <option value="{{$level->level}}" @if(optional($subordinate_joinning)->level == $level->level) selected @endif>{{$level->name}}</option>
                                            @endforeach
                                        </select>
                                        @error('level')
                                            <span class="text-danger">{{$message}}</span>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="tier" id="tier" class="form-control select" onchange="getCommissionTable()" required>
                                            @for ($i = 1; $i <= 10; $i++)
                                                <option value="{{$i}}" @if(optional($subordinate_joinning)->tier == $i) selected @endif>{{$i}}</option>
                                            @endfor
                                        </select>
                                        @error('tier')
                                            <span class="text-danger">{{$message}}</span>
                                    ...
                                    <input type="hidden" name="status" id="status" value="{{$subordinate_joinning?$subordinate_joinning->status:'0'}}">
                                </div>
    ...
    @push('js')
        ...
        <script>
            ...
            function getCommissionTable(){
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('admin.subordinate.joinning.table') }}",
                    data: {
                        _token:"{{csrf_token()}}",
            ...
            function getJoinningStatus(){
                ...
                if($('#subordinate_joinning_status').is(":checked")){
